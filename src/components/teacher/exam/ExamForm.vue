  ...
  <div>
    <el-form size="middle"
      ...
      <el-form-item label="答案">
        <div>
          <el-input v-for="(item,id) in inputElement"
                    :key="id"
                    v-model="editForm.answer[item.id]"
                    :placeholder="item.placeholder"
                    class="tw-exam--answer"></el-input>
          <el-button type="primary"
                     @click="addEle">添加答案</el-button>
                            <el-button type="primary"
                     @click="subEle">减少答案</el-button>
        </div>
...
export default {
  ...
  computed: {
    ...
    inputElement() {
      if (this.isUpdate) {
        let length = Object.keys(this.editForm.answer).length
        let len = this.eleItem.length
  ...
  methods: {
    ...
    async submitForm() {
      try {
        ...
        this.editForm.answer = JSON.stringify(this.editForm.answer);
