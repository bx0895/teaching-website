<template>
  <div>
    <p v-if="showMsg" class="tw-msg"><span class="tw-msg--title">编程题：</span>录入编程题到题库中</p>
    <el-form size="middle"
             label-width="80px"
             ref="editForm"
             :model="editForm">
      ...
      <el-radio-group v-model="editForm.submit">
       <el-radio label="text">文本提交</el-radio> 
      <el-radio label="file">源文件提交</el-radio>
    </el-radio-group>
      </el-form-item>
      <p class="tw-exam--title">程序内容</p>
...
export default {
    ...
    isUpdate:{
     ...
     editForm:{
      ...
      default:function(){
        return   {
      title: "",
      content: "<p>题目要求：</p><p>输入描述:：</p><p>输出描述：</p><p>输入例子:：</p><p>输出例子：</p>",
      difficulty: null,
      answer: "",
      analysis: "",
      label: "",
  ...
  methods: {
    ...
    async submitForm() {
      try {
        ...
        this.editForm.answer = this.editor2.txt.html();
  ...
  mounted() {
    ...
    this.editor2 = new Editor(this.$refs.answer);
    this.seteditor(this.editor1, this.editForm.content);
    this.seteditor(this.editor2,this.editForm.answer);
  },
};
</script>
<style lang='stylus'>
