...
import TestModal from "@/components/teacher/common/Modal";
...
export default {
  ...
  methods: {
    ...
    handleCorrect(row) {
      this.$router.push({path:'/correct/test',query:{cla_id:row.cla_id,test_id:row.id}})
    },
    handleEdit(row) {
      delete row.cla_name
      delete row.tea_id
      ...
      this.$store.commit("modal/CHANGE_TYPE", "test");
    ...
    async getTableInfo() {
      try {
        let res = await this.$api({type:'testInfo',data:{q:{tea_id:store.get('uid')}}})
         res.result.map(item => {
